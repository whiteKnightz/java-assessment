...
public class Dota2Senate {
    public static String predictPartyVictory(String senate) {
        ...
        Queue<Integer> indexOfR = new LinkedList<>();
        Queue<Integer> indexOfD = new LinkedList<>();
        for (int i = 0; i < len; i++) {
        ...
        while (!indexOfD.isEmpty() && !indexOfR.isEmpty()) {
            Integer dIndex = indexOfD.poll();
            Integer rIndex = indexOfR.poll();
            if (rIndex<dIndex){
                indexOfR.offer(rIndex+len);
            } else {
                indexOfD.offer(dIndex+len);
            }
        }
        return indexOfD.size()>0?"Dire":"Radiant";
    }

    public static void main(String[] args) {
        String senate;
        senate = "RD";
        System.out.println("Result for "+senate+":"+predictPartyVictory(senate));
        senate = "RDD";
        System.out.println("Result for "+senate+":"+predictPartyVictory(senate));
